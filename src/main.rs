...
/// Configure the winit event loop to suppress Windows error dings from control
/// WM_CHAR messages generated for shortcuts like Ctrl+C. Those control chars
/// are not useful to egui text input, and letting them reach the default window
/// proc can trigger MessageBeep(0) on Windows.
///
