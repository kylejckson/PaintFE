...
impl ColorsPanel {
    ...
    fn sync_hsv_from_colors(&mut self) {
        let exp = hsv_to_color(self.primary_hsv[0], self.primary_hsv[1],
                               self.primary_hsv[2], self.primary_color.a());
        ...
        let exp = hsv_to_color(self.secondary_hsv[0], self.secondary_hsv[1],
                               self.secondary_hsv[2], self.secondary_color.a());
    ...
    fn draw_swatches(&mut self, ui: &mut egui::Ui, assets: &Assets) {
        ui.horizontal(|ui| {
            ...
            if pri_resp.clicked() { self.editing_primary = true; }
            ...
            if sec_resp.clicked() { self.editing_primary = false; }

            // -- swap button --
            if assets.small_icon_button_frameless(ui, Icon::SwapColors).clicked() {
                self.swap_colors();
            }

            // -- spacer + expand / collapse toggle  (right-aligned) --
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let icon = if self.expanded { Icon::Collapse } else { Icon::Expand };
                let tip  = if self.expanded { "Compact mode" } else { "Show sliders" };
                if assets.small_icon_button(ui, icon).on_hover_text(tip).clicked() {
    ...
    fn draw_hue_ring_and_sv_triangle(&mut self, ui: &mut egui::Ui) {
        ...
        let (rect, response) =
            ui.allocate_exact_size(widget_size, egui::Sense::click_and_drag());
        let center = rect.center();

        // -- copy out current values (avoids borrow issues) --
        let mut h = if self.editing_primary { self.primary_hsv[0] } else { self.secondary_hsv[0] };
        let mut s = if self.editing_primary { self.primary_hsv[1] } else { self.secondary_hsv[1] };
        let mut v = if self.editing_primary { self.primary_hsv[2] } else { self.secondary_hsv[2] };
        let alpha  = if self.editing_primary { self.primary_color.a() } else { self.secondary_color.a() };
        let mut drag_zone = self.drag_zone;
        let mut changed = false;

        // -- triangle vertex positions (rotate with hue) --
        let hue_angle = h * TAU;
        let vert_a = Pos2::new(center.x + hue_angle.cos()                       * tri_r,
                               center.y + hue_angle.sin()                       * tri_r);
        let angle_b = hue_angle + TAU / 3.0;
        let vert_b = Pos2::new(center.x + angle_b.cos() * tri_r,
                               center.y + angle_b.sin() * tri_r);
        let angle_c = hue_angle + 2.0 * TAU / 3.0;
        let vert_c = Pos2::new(center.x + angle_c.cos() * tri_r,
        ...
        if ui.is_rect_visible(rect) {
            ...
            for i in 0..segs {
                ...
                ring.colored_vertex(Pos2::new(center.x + a0.cos() * (outer_r + aa_w),
                                              center.y + a0.sin() * (outer_r + aa_w)), c0t);
                ring.colored_vertex(Pos2::new(center.x + a1.cos() * (outer_r + aa_w),
                                              center.y + a1.sin() * (outer_r + aa_w)), c1t);
                // outer solid edge
                ring.colored_vertex(Pos2::new(center.x + a0.cos() * outer_r,
                                              center.y + a0.sin() * outer_r), c0);
                ring.colored_vertex(Pos2::new(center.x + a1.cos() * outer_r,
                                              center.y + a1.sin() * outer_r), c1);
                // inner solid edge
                ring.colored_vertex(Pos2::new(center.x + a0.cos() * inner_r,
                                              center.y + a0.sin() * inner_r), c0);
                ring.colored_vertex(Pos2::new(center.x + a1.cos() * inner_r,
                                              center.y + a1.sin() * inner_r), c1);
                // inner AA fringe (opaque → transparent)
                ring.colored_vertex(Pos2::new(center.x + a0.cos() * (inner_r - aa_w),
                                              center.y + a0.sin() * (inner_r - aa_w)), c0t);
                ring.colored_vertex(Pos2::new(center.x + a1.cos() * (inner_r - aa_w),
                                              center.y + a1.sin() * (inner_r - aa_w)), c1t);
                // outer fringe quad
            ...
            let tri_verts = [(vert_a, pure_col), (vert_b, Color32::WHITE), (vert_c, Color32::BLACK)];
            for i in 0..3 {
                ...
                let n = if n.x * to_opp.x + n.y * to_opp.y > 0.0 { -n } else { n };
                let c0t = Color32::from_rgba_unmultiplied(c0.r(), c0.g(), c0.b(), 0);
                let c1t = Color32::from_rgba_unmultiplied(c1.r(), c1.g(), c1.b(), 0);
            ...
            let line_inner = Pos2::new(center.x + hue_angle.cos() * (inner_r - 1.0),
                                       center.y + hue_angle.sin() * (inner_r - 1.0));
            let line_outer = Pos2::new(center.x + hue_angle.cos() * (outer_r + 1.0),
                                       center.y + hue_angle.sin() * (outer_r + 1.0));
            // dark outline for contrast
            p.line_segment([line_inner, line_outer],
                ...
                match drag_zone {
                    ...
                    DragZone::SvTriangle => {
                        ...
                        if sum > 0.001 {
                            ...
                            let new_s = if new_v > 0.001 { (wa / new_v).clamp(0.0, 1.0) } else { 0.0 };
                            s = new_s;
                            v = new_v;
                            changed = true;
                        }
    ...
    fn draw_alpha_slider(&mut self, ui: &mut egui::Ui) {
        ...
        let hsv = if editing { self.primary_hsv } else { self.secondary_hsv };
        let color = if editing { self.primary_color } else { self.secondary_color };
        ...
        ui.horizontal(|ui| {
            ...
            if ui.is_rect_visible(rect) {
                ...
                for i in 0..steps {
                    ...
                    p.rect_filled(
                        egui::Rect::from_min_max(Pos2::new(x0, bar.min.y),
                                                 Pos2::new(x1, bar.max.y)),
                        0.0,
                        Color32::from_rgba_unmultiplied(base.r(), base.g(), base.b(),
                                                       (t * 255.0) as u8),
                    );
                }
                ...
                p.circle_stroke(Pos2::new(tx, ty), tr,
            ...
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut alpha_val)
                .clamp_range(0..=255).speed(1)).changed()
            {
        ...
        if changed {
            ...
            let [ch, cs, cv] = if editing { self.primary_hsv } else { self.secondary_hsv };
            if editing {
    ...
    fn draw_hex_row(&mut self, ui: &mut egui::Ui, assets: &Assets) {
        let editing = self.editing_primary;
        let a = if editing { self.primary_color.a() } else { self.secondary_color.a() };
        ...
        ui.horizontal(|ui| {
            ...
            if assets.small_icon_button_frameless(ui, Icon::CopyHex).clicked() {
    ...
    fn draw_rgb_sliders(&mut self, ui: &mut egui::Ui) {
        ...
        let a = if editing { self.primary_color.a() } else { self.secondary_color.a() };
        // Derive un-premultiplied RGB from HSV (Color32 stores premultiplied)
        let hsv_cur = if editing { self.primary_hsv } else { self.secondary_hsv };
        ...
        // -- R --
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("R").small().color(Color32::from_rgb(210, 90, 90))));
            let g2 = g; let b2 = b;
            if Self::gradient_bar(ui, &mut r, |t| {
                Color32::from_rgb((t * 255.0) as u8, (g2 * 255.0) as u8, (b2 * 255.0) as u8)
            }, sw, sh) { changed = true; }
            let mut ri = (r * 255.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut ri)
                .clamp_range(0..=255).speed(1)).changed() {
                r = ri as f32 / 255.0; changed = true;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("G").small().color(Color32::from_rgb(80, 190, 80))));
            let r2 = r; let b2 = b;
            if Self::gradient_bar(ui, &mut g, |t| {
                Color32::from_rgb((r2 * 255.0) as u8, (t * 255.0) as u8, (b2 * 255.0) as u8)
            }, sw, sh) { changed = true; }
            let mut gi = (g * 255.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut gi)
                .clamp_range(0..=255).speed(1)).changed() {
                g = gi as f32 / 255.0; changed = true;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("B").small().color(Color32::from_rgb(90, 120, 220))));
            let r2 = r; let g2 = g;
            if Self::gradient_bar(ui, &mut b, |t| {
                Color32::from_rgb((r2 * 255.0) as u8, (g2 * 255.0) as u8, (t * 255.0) as u8)
            }, sw, sh) { changed = true; }
            let mut bi = (b * 255.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut bi)
                .clamp_range(0..=255).speed(1)).changed() {
                b = bi as f32 / 255.0; changed = true;
            }
        });
    ...
    fn draw_hsv_sliders(&mut self, ui: &mut egui::Ui) {
        let editing = self.editing_primary;
        let hsv = if editing { self.primary_hsv } else { self.secondary_hsv };
        let alpha = if editing { self.primary_color.a() } else { self.secondary_color.a() };
        let mut h = hsv[0];
        let mut s = hsv[1];
        let mut v = hsv[2];
        let mut changed = false;
        ...
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("H").small()));
            if Self::gradient_bar(ui, &mut h, |t| {
                hsv_to_color(t, 1.0, 1.0, 255)
            }, sw, sh) { changed = true; }
            let mut hi = (h * 360.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut hi)
                .clamp_range(0..=360).speed(1).suffix("°")).changed() {
                h = hi as f32 / 360.0; changed = true;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("S").small()));
            let h2 = h; let v2 = v;
            if Self::gradient_bar(ui, &mut s, |t| {
                hsv_to_color(h2, t, v2, 255)
            }, sw, sh) { changed = true; }
            let mut si = (s * 100.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut si)
                .clamp_range(0..=100).speed(1).suffix("%")).changed() {
                s = si as f32 / 100.0; changed = true;
            }
        });
        ...
        ui.horizontal(|ui| {
            ui.add_sized([12.0, 16.0], egui::Label::new(
                egui::RichText::new("V").small()));
            let h2 = h; let s2 = s;
            if Self::gradient_bar(ui, &mut v, |t| {
                hsv_to_color(h2, s2, t, 255)
            }, sw, sh) { changed = true; }
            let mut vi = (v * 100.0).round() as u32;
            if ui.add_sized([32.0, 16.0], egui::DragValue::new(&mut vi)
                .clamp_range(0..=100).speed(1).suffix("%")).changed() {
                v = vi as f32 / 100.0; changed = true;
            }
        });
        ...
        for col in 0..cols {
            if (row + col) % 2 == 1 {
                let cr = egui::Rect::from_min_size(
                    Pos2::new(rect.min.x + col as f32 * cell,
