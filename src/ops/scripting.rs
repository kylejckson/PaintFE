...
use image::{RgbaImage, GrayImage, imageops};
use rhai::{Engine, Scope, AST, Dynamic, EvalAltResult, Array, FnPtr, ImmutableString};
...
pub enum CanvasOpRequest {
    ...
    Rotate90CCW,
    Rotate180,
    ResizeImage { w: u32, h: u32, filter: ScriptFilterType },
    ResizeCanvas { new_w: u32, new_h: u32, anchor: (u32, u32) },
}
...
impl ScriptError {
    ...
    pub fn friendly_message(&self) -> String {
        ...
        if raw.contains("Function not found:") {
            ...
            if let Some(fn_part) = raw.strip_prefix("Function not found: ") {
                ...
                if fn_name.len() <= 3 || fn_name.chars().all(|c| c.is_lowercase() || c == '_') {
                    ...
                    parts.push("  Tip: If this is a closure stored in a variable, use .call() syntax:".to_string());
            ...
            if let Some(var_part) = raw.split("Variable not found:").nth(1) {
                ...
                parts.push(String::new());
                parts.push("  Tip: Make sure you declared it with 'let' before using it:".to_string());
                parts.push(format!("    let {} = 0;", var_name));
            } else {
                parts.push(format!("  {}", raw));
            }
        } else if raw.contains("Syntax error") || raw.contains("Expected") {
            parts.push(format!("  Syntax error: {}", raw.split(" (line ").next().unwrap_or(raw)));
            parts.push(String::new());
            parts.push("  Tip: Check for missing semicolons, brackets, or typos near this line.".to_string());
        } else if raw.contains("Data type") && raw.contains("not supported") {
            parts.push(format!("  Type error: {}", raw.split(" (line ").next().unwrap_or(raw)));
            parts.push(String::new());
            parts.push("  Tip: Rhai is strict about types. Use .to_float() or .to_int() to convert:".to_string());
            parts.push("    let x = 10;           // int".to_string());
            parts.push("    let y = x.to_float(); // now float".to_string());
        } else if raw.contains("Too many operations") {
            parts.push("  Script exceeded the maximum operation limit (50 million ops).".to_string());
            parts.push(String::new());
            parts.push("  Tip: Your script may have an infinite loop, or is processing".to_string());
            parts.push("  too many pixels. Try processing a smaller region with for_region(),".to_string());
        ...
        } else if raw.contains("ErrorTerminated") || raw.contains("terminated") || raw.contains("Script execution cancelled") {
...
fn register_pixel_api(engine: &mut Engine, ctx: SharedContext) {
    ...
    let c = ctx.clone();
    engine.register_fn("set_pixel", move |x: i64, y: i64, r: i64, g: i64, b: i64, a: i64| {
    ...
    for (name, offset) in [("get_r", 0usize), ("get_g", 1), ("get_b", 2), ("get_a", 3)] {
        ...
        engine.register_fn(name, move |x: i64, y: i64| -> i64 {
            ...
            if idx < lock.pixels.len() { lock.pixels[idx] as i64 } else { 0 }
        });
    }
    ...
    let c = ctx.clone();
    engine.register_fn("for_each_pixel", move |ncc: rhai::NativeCallContext, callback: FnPtr| -> Result<(), Box<EvalAltResult>> {
        ...
        for y in 0..h as i64 {
            for x in 0..w as i64 {
                ...
                let result = callback.call_within_context::<Dynamic>(
                    &ncc,
                    (x, y, r, g, b, a),
                )?;
    ...
    let c = ctx.clone();
    engine.register_fn("for_region", move |ncc: rhai::NativeCallContext, rx: i64, ry: i64, rw: i64, rh: i64, callback: FnPtr| -> Result<(), Box<EvalAltResult>> {
        let (w, h, mut pixels) = {
            let lock = c.lock().unwrap_or_else(|e| e.into_inner());
            (lock.width, lock.height, lock.pixels.clone())
        };
    ...
    let c = ctx.clone();
    engine.register_fn("map_channels", move |ncc: rhai::NativeCallContext, callback: FnPtr| -> Result<(), Box<EvalAltResult>> {
        let (w, h, mut pixels) = {
            let lock = c.lock().unwrap_or_else(|e| e.into_inner());
            (lock.width, lock.height, lock.pixels.clone())
        };
...
fn apply_effect_to_context(ctx: &SharedContext, f: impl FnOnce(&RgbaImage, Option<&GrayImage>) -> RgbaImage) {
    let mut lock = ctx.lock().unwrap_or_else(|e| e.into_inner());
    let w = lock.width;
    let h = lock.height;
    let img = RgbaImage::from_raw(w, h, lock.pixels.clone())
        .unwrap_or_else(|| RgbaImage::new(w, h));

    let mask = lock.mask.as_ref().map(|m| {
        GrayImage::from_raw(w, h, m.clone()).unwrap_or_else(|| GrayImage::new(w, h))
    });
...
fn register_transform_api(engine: &mut Engine, ctx: SharedContext) {
    ...
    let c = ctx.clone();
    engine.register_fn("resize_image", move |new_w: i64, new_h: i64, method: ImmutableString| {
        ...
        lock.canvas_ops.push(CanvasOpRequest::ResizeImage { w: new_w, h: new_h, filter });
    });
    ...
    let c = ctx.clone();
    engine.register_fn("resize_canvas", move |new_w: i64, new_h: i64, anchor: ImmutableString| {
        let new_w = (new_w.max(1) as u32).min(32768);
        let new_h = (new_h.max(1) as u32).min(32768);
        ...
        lock.canvas_ops.push(CanvasOpRequest::ResizeCanvas { new_w, new_h, anchor: anchor_tuple });
    });
}
...
fn register_effect_api(engine: &mut Engine, ctx: SharedContext) {
    ...
    let c = ctx.clone();
    engine.register_fn("apply_brightness_contrast", move |brightness: f64, contrast: f64| {
        let mut lock = c.lock().unwrap_or_else(|e| e.into_inner());
        ...
        while i + 3 < len {
            ...
            lock.pixels[i + 1] = (factor * (g + bright - 128.0) + 128.0).clamp(0.0, 255.0) as u8;
            lock.pixels[i + 2] = (factor * (b + bright - 128.0) + 128.0).clamp(0.0, 255.0) as u8;
            i += 4;
        }
    });
    ...
    engine.register_fn("apply_hsl", move |hue: f64, sat: f64, light: f64| {
        ...
        while i + 3 < len {
            ...
            let (h, s) = if (cmax - cmin).abs() < 1e-10 {
                ...
                let d = cmax - cmin;
                let s = if l > 0.5 { d / (2.0 - cmax - cmin) } else { d / (cmax + cmin) };
            ...
            let (nr, ng, nb) = if ns.abs() < 1e-10 {
                ...
                let q = if l < 0.5 { l * (1.0 + ns) } else { l + ns - l * ns };
                let p = 2.0 * l - q;
                ...
                (hue2rgb(p, q, nh + 1.0 / 3.0), hue2rgb(p, q, nh), hue2rgb(p, q, nh - 1.0 / 3.0))
            };
    ...
    engine.register_fn("apply_noise", move |amount: f64, monochrome: bool| {
        apply_effect_to_context(&c, |img, mask| {
            crate::ops::effects::add_noise_core(img, amount as f32, crate::ops::effects::NoiseType::Gaussian, monochrome, 42, 1.0, 1, mask)
        });
    });
    ...
    engine.register_fn("apply_halftone", move |dot_size: f64| {
        apply_effect_to_context(&c, |img, mask| {
            crate::ops::effects::halftone_core(img, dot_size as f32, 45.0, crate::ops::effects::HalftoneShape::Circle, mask)
        });
    });
...
fn register_utility_api(engine: &mut Engine, ctx: SharedContext) {
    ...
    engine.register_fn("print_line", move |msg: ImmutableString| {
        let lock = c.lock().unwrap_or_else(|e| e.into_inner());
        let _ = lock.sender.send(ScriptMessage::ConsoleOutput(msg.to_string()));
    });
    ...
    engine.register_fn("progress", move |frac: f64| {
        let lock = c.lock().unwrap_or_else(|e| e.into_inner());
        let _ = lock.sender.send(ScriptMessage::Progress(frac.clamp(0.0, 1.0) as f32));
    });
    ...
    engine.register_fn("clamp_f", |v: f64, lo: f64, hi: f64| -> f64 { v.clamp(lo, hi) });
...
pub fn compile_script(source: &str) -> Result<AST, ScriptError> {
    ...
    engine.compile(source).map_err(|e| {
        ...
        ScriptError {
            message: e.to_string(),
            line: if pos.line().unwrap_or(0) > 0 { Some(pos.line().unwrap_or(0)) } else { None },
            column: if pos.position().unwrap_or(0) > 0 { Some(pos.position().unwrap_or(0)) } else { None },
        }
    })
}
    ...
    for op in canvas_ops {
        ...
        for i in 0..n {
            ...
            if let Some(img) = RgbaImage::from_raw(cur_w, cur_h, flat) {
                ...
                state.layers[i].pixels =
                    crate::canvas::TiledImage::from_rgba_image(&new_img);
            }
        }
